...
def test_cidr_ranges_not_found_as_urls():
    ...
    result = find_iocs('1.1.1.1/0', data_types=["urls"])
    assert result['urls'] == []

    result = find_iocs('1.1.1.1/0', parse_urls_without_scheme=False, data_types=["urls"])
    assert result['urls'] == []

    result = find_iocs('1.1.1.1/0 foobar.com/test/bingo.php', data_types=["urls"])
...
def test_parse_domain_from_url_not_removing_entire_url():
    ...
    # default behaviour
    result = find_iocs('https://foobar.com/test/bingo.com/bar', data_types=["urls"])
    assert iterables_have_same_items(result['domains'], ['foobar.com', 'bingo.com'])

    result = find_iocs('https://foobar.com/test/bingo.com/bar', parse_domain_from_url=False, data_types=["urls"])
    assert result['domains'] == ['bingo.com']

    result = find_iocs('https://foobar.com/test/bingo.com/bar', parse_domain_from_url=False, parse_from_url_path=False, data_types=["urls"])
...
def test_url__percent_encoded_path():
    ...
    result = find_iocs(s, parse_from_url_path=False, data_types=["urls"])
