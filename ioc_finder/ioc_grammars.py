...
md5 = file_hash_word_start + Word(hexnums, exact=32).set_parse_action(pyparsing_common.downcase_tokens) + alphanum_word_end
imphash = Combine(
    Or(['imphash', 'import hash']) + Optional(Word(printables, excludeChars=alphanums)) + md5('hash'),
    joinString=' ',
    adjacent=False,
)
sha1 = file_hash_word_start + Word(hexnums, exact=40).set_parse_action(pyparsing_common.downcase_tokens) + alphanum_word_end
sha256 = file_hash_word_start + Word(hexnums, exact=64).set_parse_action(pyparsing_common.downcase_tokens) + alphanum_word_end
authentihash = Combine(
    Or(['authentihash']) + Optional(Word(printables, excludeChars=alphanums)) + sha256('hash'),
    joinString=' ',
    adjacent=False,
)
sha512 = file_hash_word_start + Word(hexnums, exact=128).set_parse_action(pyparsing_common.downcase_tokens) + alphanum_word_end
